...
router.post('/', async (req, res) => {
    const result = await orderList(req.body);
    res.status(result.status).send(result.data);
});

const orderList = async (body: Record<string, any>) => {
    ...
    try {
        const response =  await axios.post(appConfig.basePath + apiPath.ordersList, jData + jKey);
