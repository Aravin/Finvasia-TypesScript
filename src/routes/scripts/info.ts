...
router.post('/', async (req, res) => {
    const result = await scriptInfo(req.body);
    res.status(result.status).send(result.data);
});

const scriptInfo = async (body: Record<string, any>) => {
    ...
    try {
        const response =  await axios.post(appConfig.basePath + apiPath.scriptInfo, jData + jKey);
