...
router.post('/', async (req, res) => {
    const result = await scriptQuote(req.body);
    res.status(result.status).send(result.data);
});

const scriptQuote = async (body: Record<string, any>) => {
    ...
    try {
        const response =  await axios.post(appConfig.basePath + apiPath.scriptQuote, jData + jKey);
